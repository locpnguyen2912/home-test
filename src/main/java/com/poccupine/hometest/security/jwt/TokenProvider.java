...
//    private int rememberMe; // default 24 hours
//
//    @Value("${locpnguyen.security.jwt.secret}")
//    private String secretKey;
    ...
    public TokenProvider() {
        ...
        if (!ObjectUtils.isEmpty(secret)) {
            ...
            keyBytes = Decoders.BASE64.decode(config.getSecretKey());
        }
        key = Keys.hmacShaKeyFor(keyBytes);
        jwtParser = Jwts.parserBuilder().setSigningKey(key).build();
        this.tokenValidityInMilliseconds = 1000 * config.getExpiration();
        this.tokenValidityInMillisecondsForRememberMe =
            1000 * config.getRememberMe();
    }
